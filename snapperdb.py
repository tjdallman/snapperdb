...
def run_command(args):
    ...
    if args.command == 'fastq_to_db':
        ...
        logger.info('PARAMS: config = %s' % args.config_file)
        update_clusters(args.accept, args.place, args.full, config_dict)
    
    elif args.command == 'check_clusters':
        logger = logging.getLogger('snapperdb.check_clusters')
        logger.info('PARAMS: config = %s' % args.config_file)
        check_clusters(config_dict)
    ...
    elif args.command == 'get_nlessness':
        logger = logging.getLogger('snapperdb.get_nlessness')
        logger.info('PARAMS: config = %s' % args.config_file)
        get_nlessness(args, config_dict)

    elif args.command == 'prepare_reference':
        logger = logging.getLogger('snapperdb.prepare_reference')
        logger.info('PARAMS: config = %s' % args.config_file)
        vcf = make_fastq(args, config_dict)
...
def main():
    ...
    parser.add_argument('-g', '--logdir',  dest='log_dir', default=os.getcwd(),
                                     help='Where do you want the logs written to? Will default to /logs')
    ...
    parser_update_clusters.add_argument('-g', dest='log_dir', default=os.getcwd(),
                                        help='Where do you want the logs written to? Will default to a /user/home/logs')
    parser_update_clusters.add_argument('-p', dest='place', default='N',
                                        help='Shall we just place the new isolates rather then updating the whole database')
    parser_update_clusters.add_argument('-a', dest='accept', default='N',
                                        help='List of isolates to accept merge')
    parser_update_clusters.add_argument('-f', dest='full', default='Y',
                                        help='Run with full or partial distance matrix to save memory')  
    ...
    parser_accept_outlier.add_argument('-g', dest='log_dir', default=os.getcwd(),
                                        help='Where do you want the logs written to? Will default to a /user/home/logs')   
    parser_get_nlessness = subparsers.add_parser('get_nlessness', help = 'Calculate mean nless distance and N50.')
    parser_get_nlessness.add_argument('-c', dest='config_file', metavar='Config file', required=True,
                                        help='The name of a config file in the user_configs directory (not the full path)')
    parser_get_nlessness.add_argument('-l', dest = 'strain_list', default = None, help = 'supply isolate(s) for which to do nlessness for, list (l), name (n), or all SNP typed isolates (a)')
    parser_get_nlessness.add_argument('-n', dest = 'name', default = None, help = 'supply isolate(s) for which to do nlessness for, list (l), name (n), or all SNP typed isolates (a)')
    parser_get_nlessness.add_argument('-a', dest = 'all_isolates', action = "store_true", default = False, help = 'supply isolate(s) for which to do nlessness for, list (l), name (n), or all SNP typed isolates (a)')
    ...
    parser_prepare_reference.add_argument('-g', dest='log_dir', default=os.getcwd(),
                                     help='Where do you want the logs written to? Will default to cwd')
    parser_prepare_reference.add_argument('-f', dest = 'fastqs', default = [], nargs='+',
                                     help='This should be ignored')
