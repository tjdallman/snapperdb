...
try:
    __ref_genome_dir__ = os.environ['GASTROSNAPPER_REFPATH']
except KeyError:
        ...
        with open(path_to_config, 'r') as fi:
            for line in fi.readlines():
                ...
                if line.startswith('mapper_threads'):
                    config_dict['mapper_threads'] = int(line.strip().split()[-1])
                if line.startswith('variant_caller_threads'):
                    config_dict['variant_caller_threads'] = int(line.strip().split()[-1])
    except IOError:
